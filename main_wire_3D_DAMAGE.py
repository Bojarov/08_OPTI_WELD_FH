...
node_dens = 20  # number of nodes on radius
...
# damage parameters
rd_o = Ro#-min_skin_depth
rd_i = Ri+min_skin_depth
d_weld = 0.05
l_weld = flen #0.1
phi_c_weld = np.pi / 2
...
node_dens_vec = np.array([int(node_dens), int(node_dens*1/2), int(node_dens)])  # vector of node densities in shells
...
def main():
    ...
    #phi_cv = [np.pi/2]#, np.pi/4, np.pi/2]
    #dw_v = [0.0, 0.025, 0.05, 0.075, 0.1]
    dw_v = [0.025] # , 0.05, 0.075, 0.1]
    lw_v = [flen]
    ...
    rdo_v=[Ro, Ro-0.2*thick, Ro - 0.4*thick, Ro-0.6*thick, Ri-0.8*thick]
    #rdo_v = [Ro]
    ...
    #op.b_at_det_plot_f( phi_cv, dw_v, rdi_v, rdo_v, lw_v, xd_v, yd_v, zd_v, f_v,
    #    params, damage_params, output_params, filament_params, node_dens_vec, r_sub_vec)
