...
node_dens_vec = np.array([int(node_dens*1/2), int(node_dens*1/2), int(node_dens)])  # vector of node densities in shells
...
y_detect = 1.0  # height detector position above tube
...
def main():
    ...
    phi_cv = [np.pi/2]
    #phi_cv = [0.0, np.pi/4, np.pi/2]
    dw_v   = [0.0,0.025, 0.05, 0.075, 0.1]
    #dw_v   = [0.0, 0.1]
    #dw_v   = [0.075, 0.1]
    #lw_v = [0.025]#,0.25,0.1,0.05,0.01]
    lw_v = [0.05]#,0.25,0.1,0.05,0.01]
    #lw_v = [0.075]#,0.25,0.1,0.05,0.01]
    #lw_v = [0.1]#,0.25,0.1,0.05,0.01]
    #lw_v = [1.0]#,0.25,0.1,0.05,0.01]
    
    xd_v = [-0.5,-0.4,-0.2,-0.1, 0.0, 0.1, 0.2, 0.4, 0.5]
    #xd_v = [-0.5, 0.0, 0.5]
    #xd_v = [0.0]
    yd_v = [1.0]
    zd_v = [flen/2]
    ...
    #f_v = [10**0, 100]
    thick=Ro-Ri
    #rdi_v = [Ri, Ri+0.2*thick, Ri+0.4*thick, Ri+0.6*thick, Ri+0.8*thick]
    rdi_v = [Ri]
    #rdi_v = [Ri+0.2*thick]#, Ri+0.4*thick, Ri+0.6*thick, Ri+0.8*thick]
    #rdi_v = [Ri+0.4*thick]#, Ri+0.6*thick, Ri+0.8*thick]
    #rdi_v = [Ri+0.6*thick]#, Ri+0.8*thick]
    #rdi_v = [Ri+0.8*thick]
    rdo_v = [Ro]
    ...
    op.b_at_det_plot_f1( phi_cv, dw_v, rdi_v, rdo_v, lw_v, xd_v, yd_v, zd_v, f_v,
        params, damage_params, output_params, filament_params, node_dens_vec, r_sub_vec)
