    ...
    for i in range(n_det):
        ...
        for j in range(n_f):
            ...
            for k in range(1):
                ...
                ZC_mat = np.flipud(ohz.ZC_mat_extract(geo_objects_local))
                # print(ZC_mat)
                tol = 10 ** (-16)
                ...
                # print(L23)
                # B_pass_loops
                b_at_det_f[i, j, k] = Y[1, 0]
                print(np.angle(Y[0, 0]), np.angle(Y[0, 1]), np.angle(Y[0, 1] / Y[0, 0]) / (np.pi))
                print(np.angle(Y[0, 0]), np.angle(Y[0, 2]), np.angle(Y[0, 2] / Y[0, 0] / (np.pi)))
